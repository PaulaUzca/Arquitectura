...
Parser::~Parser() {
    compDict.clear();  // eliminar todo el contenido de la tabla
    destDict.clear();
    jumpDict.clear();
}
...
string Parser::parseInst(string inst) {
    ...
    if (inst.find('@') != string::npos) {
        tempInst = inst.substr(1, inst.size() - 1);  // QUITA EL '@'
        return parseAInst(tempInst);                 // INSTRUCCIÓN a
    } else {
        return parseCInst(inst);  // INSTRUCCIÓN C
    }
}
